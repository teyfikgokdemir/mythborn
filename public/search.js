(()=>{
  ...
  let items=[];
  const render=()=>{
    const q=String(form.elements.q.value||'').trim().toLocaleLowerCase(locale==='el'?'el':'tr'),category=String(form.elements.category.value||'');
    const found=items.filter(item=>(!category||item.category===category)&&(!q||`${item.title} ${item.description}`.toLocaleLowerCase(locale==='el'?'el':'tr').includes(q)));
