...
const localDayKey=()=>{const now=new Date(),part=n=>String(n).padStart(2,'0');return`${now.getFullYear()}-${part(now.getMonth()+1)}-${part(now.getDate())}`};
...
const revealDaily=async()=>{try{const deck=await waitForDeck();let index=readDailyIndex(deck.length);if(index===null){index=secureIndex(deck.length);saveDailyIndex(index)}const card=deck[index];dailyResult.querySelector('[data-card-name]').textContent=card.name;dailyResult.querySelector('[data-card-text]').innerHTML=cardBody(card);dailyResult.classList.add('is-open');dailyButton.hidden=true;dailyButton.setAttribute('aria-hidden','true');dailyButton.setAttribute('aria-expanded','true')}catch(error){dailyResult.classList.add('is-open');dailyResult.querySelector('[data-card-name]').textContent='Deste hazırlanamadı';dailyResult.querySelector('[data-card-text]').innerHTML=`<p>${escape(error.message)} Sayfayı yenileyip tekrar dene.</p>`}};
