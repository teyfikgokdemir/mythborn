...
for(const file of ['public/oracle.css','public/oracle.js','public/tarot-deck.js','public/synastry.js','src/auth.js','src/account.js','src/astrology.js'])await access(new URL(`../${file}`,import.meta.url));
...
const sitemap=await check('/sitemap.xml',200,['https://mythborn.co/gunluk-kart','https://mythborn.co/katina','https://mythborn.co/astroloji','https://mythborn.co/haftalik-burc','https://mythborn.co/sinastri']);
if(sitemap.body.includes('/giris')||sitemap.body.includes('/kayit'))throw new Error('Private routes leaked into sitemap');
await check('/llms.txt',200,['78 kartlık klasik Tarot destesi','gerçek astronomik veriler','Sinastri']);
...
console.log('Mythborn Tarot, doğum haritası, sinastri, güncel gökyüzü, haftalık burç, üyelik, SEO ve route audit’i geçti.');
